...
class CoDeepNEATModuleConv2D(CoDeepNEATModuleBase):
    """
    TFNE CoDeepNEAT module encapsulating a Conv2D layer. The downsampling layer is another Conv2D layer.
    """
    ...
    def __str__(self) -> str:
        ...
        """
        return "CoDeepNEAT Conv2D Module | ID: {:>6} | Fitness: {:>6} | Filters: {:>4} | " \
               "Kernel: {:>6} | Activ: {:>6}" \
    ...
    def create_downsampling_layer(self, in_shape, out_shape) -> tf.keras.layers.Layer:
        ...
        """
        # As the Conv2D module downsamples with a Conv2D layer, assure that the input and output shape
    ...
    def create_crossover(self,
                         ...
                         max_degree_of_mutation) -> CoDeepNEATModuleConv2D:
        """
        Create crossed over Conv2D module and return it. Carry over parameters of fitter parent for
        categorical parameters and calculate parameter average between both modules for sortable parameters
        @param offspring_id: int of unique module ID of the offspring
        @param less_fit_module: second Conv2D module with lower fitness
        @param max_degree_of_mutation: float between 0 and 1 specifying the maximum degree of mutation
        @return: instantiated Conv2D module with crossed over parameters
        """
    ...
    def get_distance(self, other_module) -> float:
        """
        Calculate distance between 2 Conv2D modules by inspecting each parameter, calculating the
        ...
        @param other_module: second Conv2D module to which the distance has to be calculated
        @return: float between 0 and 1. High values indicating difference, low values indicating similarity
        """
        if not isinstance(other_module, CoDeepNEATModuleConv2D):
            return 0.0
